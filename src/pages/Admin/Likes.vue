  ...
  <div>
    <div v-if="statsDataLikes" class="admin__pages-likes panel">
      <div class="panel-publications__top">
        <img src="../../../public/static/img/admin/admin_4.png" alt="Комментарии">
...
export default defineComponent({
  ...
  computed: {
    dynamics() {
      ...
      const xAxisData = this.statsDataLikes?.countPerMonth.map(item => {
        const date = new Date(item.date);
        return months[date.getMonth()];
      });
      const seriesData = this.statsDataLikes?.countPerMonth.map(item => item.count);
    ...
    dynamicsLikes() {
      return {
        ...
        xAxis: {
          type: 'category',
          data: this.statsDataLikes?.countPerHours.map(({ date }) => date.slice(11, 13)),
          splitLine: {
            show: true,
          },
        },
        ...
        series: [
          {
            data: this.statsDataLikes?.countPerHours.map(({ count }) => count),
            type: 'bar',
            smooth: true,
            barWidth: '13px',
            color: '#21A45D'
